...
o.data("UserGraphs", () => ({
  ...
  getCategoryBreakdown() {
    ...
    const i = [];
    for (const s in t) {
      const r = Number((t[s] / a.length) * 100).toFixed(2);
      i.push({ name: s, count: t[s], color: n(s), percent: r });
    }
    return i;
  },
  async init() {
    (this.solves = await e.pages.users.userSolves("me")),
      (this.fails = await e.pages.users.userFails("me")),
      (this.awards = await e.pages.users.userAwards("me")),
      ...
      u(
        ...
        l(e.user.id, e.user.name, this.solves.data, this.awards.data)
      );
  },
}));
o.start();
